...
namespace adsvel::log {
    ...
    class FileSink : public BaseSink {
       public:
        FileSink(LogLevels in_log_level, const string in_file_name_pattern, size_t in_max_log_file_size_mb, size_t in_amount_of_log_files) : file_name_pattern_{in_file_name_pattern}, log_level_{in_log_level}, max_log_file_size_mb_{in_max_log_file_size_mb}, amount_of_log_files_{in_amount_of_log_files} {
        ...
        void Log(const LogMessage& in_msg) override final {
            if (log_level_ <= in_msg.level) {
                ...
                line.append(".");
                line.append(fmt::format("{}", std::chrono::duration_cast<std::chrono::milliseconds>(in_msg.time.time_since_epoch()).count() % 1000));
                line.append("][");
                line.append(LogLevelsStr.at(static_cast<uint16_t>(in_msg.level)));
                line.append(in_msg.message);
       ...
       private:
        ...
        void OpenRelevantLogFile_() {
            ...
            while (true) {
                ...
                logs_file_full_name_ = MakeLogsFileFullName_(counter);
                if (counter == 0) {
                    logs_file_full_name_ = MakeLogsFileFullName_(1);
                    logs_file_stream_.open(logs_file_full_name_, std::ofstream::out | std::ofstream::app);
                    current_size_of_log_file_ = 0;
                    return;
                }
